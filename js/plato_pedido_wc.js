document.addEventListener("DOMContentLoaded", function() {
  class PlatoPedidoElement extends HTMLElement {
      constructor() {
          ...
          this.shadow.querySelector('.botonAniadir').addEventListener('click', () => {
              this.cambiarCantidad(1);
          });

          this.shadow.querySelector('.botonQuitar').addEventListener('click', () => {
              this.cambiarCantidad(-1);
          });
      }
      ...
      attributeChangedCallback(name, oldValue, newValue) {
          if (name === 'data-cantidad-plato') {
              ...
              this.shadow.querySelector('.cantidad').textContent = newValue;
          }
          if (name === 'data-precio-plato-total') {
              this.precioPlatoTotal = newValue;
              this.shadow.querySelector('.precio').textContent = newValue + " €";
          }
      }
